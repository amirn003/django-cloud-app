...
def show_exam_result(request, course_id, submission_id):
    ...
    choices = Choice.objects.all().filter(is_correct=True).count()

    score = 0
    correct = submission.choices.all().filter(is_correct=True).count()
    all = submission.choices.all().count()

    if all <= correct:
        score = correct / choices * 100
    else:
        diff = all - correct
        penality = 1 / Choice.objects.all().count()
        score = (correct / choices * 100) - (penality*diff * 100)
